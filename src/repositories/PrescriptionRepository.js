...
class PrescriptionRepository {
  ...
  async create(prescription) {
    const {
      ...
      pd, intermediateAdd, nearPd, remarks, prescriptionType
    } = prescription;
    ...
    const [result] = await db.query(
      `INSERT INTO prescriptions 
       (branch_id, customer_id, optometrist_id, prescription_date, expiry_date,
        od_sph, od_cyl, od_axis, od_add, od_prism, od_base,
        os_sph, os_cyl, os_axis, os_add, os_prism, os_base,
        pd, intermediate_add, near_pd, remarks, prescription_type) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        ...
        pd || 62.00, intermediateAdd !== undefined ? intermediateAdd : 0.00, nearPd !== undefined ? nearPd : 0.00, remarks || null, prescriptionType || 'single'
      ]
    );
  ...
  async update(id, prescription) {
    ...
    await db.query(
      ...
      [
        ...
        od_sph, od_cyl, od_axis, od_add, od_prism, od_base,
        os_sph, os_cyl, os_axis, os_add, os_prism, os_base,
        pd, intermediateAdd, nearPd, remarks, prescriptionType, id
      ]
    );
